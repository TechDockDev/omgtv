        ...
        items: [
            {
                ...
                series: {
                    ...
                    isAudioSeries: false,
                    heroImageUrl: "http://img.com/series_hero.jpg"
                },
                ...
                durationSeconds: 100,
                heroImageUrl: "http://img.com/ep_hero.jpg",
            ...
            },
            {
                ...
                series: {
                    ...
                    isAudioSeries: true,
                    heroImageUrl: "http://img.com/series_hero2.jpg"
                },
                playback: { status: "READY", variants: [] },
                localization: { captions: [], availableLanguages: [] },
                ratings: { average: 4 },
                personalization: { reason: "recent" },
                durationSeconds: 100,
                heroImageUrl: "http://img.com/ep_hero2.jpg",
                defaultThumbnailUrl: null
            }
        ],
        nextCursor: null,
...
async function run() {
    ...
    const result = await service.getHomeExperience({ tag: "popular" });

    console.log("Top 10 Length:", result.data.top10?.length);
    console.log("Carousel Length:", result.data.carousel?.length);
    console.log("Continue Watch Length:", result.data["continue watch"]?.length);
    console.log("Sections:", result.data.sections.map(s => s.title));
    ...
    if (result.data.carousel && result.data.carousel.length > 0) {
        console.error("FAIL: Carousel should be empty when tag is present");
        failed = true;
    }

    if (result.data["continue watch"] && result.data["continue watch"].length > 0) {
        console.error("FAIL: Continue Watch should be empty when tag is present");
        failed = true;
    }
    ...
    if (failed) {
        ...
        console.log("SUCCESS: Behavior matches strict tag filtering expectations.");
    }
}
